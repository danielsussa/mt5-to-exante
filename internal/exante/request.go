...
func (j JWTAuthMethod) GetJWT() string {
	isValidToken := j.validateJwtToken()
	if !isValidToken {
		currentJwtAuthToken = j.getJWTToken()
	}
	authValue := fmt.Sprintf("%s %s", jwtAuthTokenPrefix, currentJwtAuthToken)
	return authValue
}
...
func (h HTTPApi) post(req interface{}, u requestData) error {
	objByte, err := h.preparePostPayload(req)
	if err != nil {
		return err
	}
	err = h.fetch(http.MethodPost, req, u, objByte)
	return err
}

func (h HTTPApi) postWithResponse(req interface{}, res interface{}, u requestData) error {
	objByte, err := h.preparePostPayload(req)
	if err != nil {
		return err
	}
	err = h.fetch(http.MethodPost, res, u, objByte)
	return err
}
